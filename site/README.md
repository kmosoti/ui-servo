...
site/
  ...
  assets/islands/       loader.js (hand-written) + GENERATED wasm-pack output
  islands/              the WASM island crate — see "Islands"
...
| `/assets/*`        | `tokens.css`, `site.css`, `motion.json`, `htmx.min.js`, `probe.js`, `favicon.svg`, `kennedy-mosoti-resume.pdf`, `islands/*` (loader + wasm-pack output) |

Fragment names: `dispatch`, `project-card`, `reading-log`, `colophon`,
`constellation`. An unknown name is a 404, never a panic — the name comes from a
URL.
...
Slots and swaps: `pages::slot(id, body)` wraps a swap target in a div that
carries its own span id, and `pages::reswap_button(...)` re-renders into it with
`hx-swap="innerHTML"` — the slot survives the swap, so `htmx:afterSwap` always
fires on an element that already has a join key.

**Nothing is mounted through them today.** The only fragment on a page is the
promoted hero on `/`, which is rendered straight into the page wrapper and
carries its own span id from `fragments::frame`. The named fragments are demo
content, and mounting them under real work-history prose would put invented
material back on the page. Both helpers stay because they are how the *next*
fragment gets swapped, not because something is being swapped now.

## Islands

`/about` mounts one WASM island: `<resume-sandbox>`, an editor that validates
pasted résumé JSON and re-renders a preview from it. The element is
server-rendered with a one-line fallback inside it, and `assets/islands/loader.js`
— loaded on *every* page, so an island htmx swaps in later still upgrades —
defines the tag and fetches the wasm lazily on first connect. A successful mount
replaces the fallback with the editor. A module that never arrives leaves the
fallback exactly where it was and reports `ui-servo:ce-error` with phase
`wasm-init`; a mount that throws after that reports phase `connect` instead, and
by then the fallback has already been replaced by however much of the editor got
built. Either way the probe attributes the report to the nearest
`[data-span-id]` ancestor. That ancestor is the page wrapper, which is why the
...
`/about` at 375px in every state the panel has.

The crate is `site/islands/` (wasm-pack → `assets/islands/`); it has its own
README, and `tests/test_island.py` is the browser-level acceptance for the whole
chain. `<ui-constellation>` is defined by the same loader but **no page mounts
it** — it is a fragment (`/fragments/constellation`) with nowhere to live yet.
