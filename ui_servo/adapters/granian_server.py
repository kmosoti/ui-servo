...
* **The port is chosen here, not by the kernel at bind time.** Granian rebuilds
  ...
  in which something else could take it; on a dev box handing out ephemeral
  ports in rotation that is not a real risk, and a lost race fails loudly at
  startup rather than quietly serving the wrong thing.
* **Readiness is a completed TCP connection.** Granian exposes no "started"
  ...
  during startup raises the reason instead of running out the clock.
"""
...
def serve_forever(app: Any, *, host: str = LOCALHOST, port: int = 8000) -> None:
    ...
    """
    server = Server(app, address=host, port=port, interface=Interfaces.ASGI)
...
def serve(
    ...
    *port* defaults to a free one. The server is stopped and joined on the way
    out, including when the body raises: a leaked server holds the port and the
    evidence store's lock, and the next round would then fail for a reason that
    has nothing to do with the next round.
    """
    bind = free_port(host) if port is None else port
    server = Server(
        app,
        address=host,
        port=bind,
    ...
    thread = threading.Thread(target=run, name=f"granian-{bind}", daemon=True)
    thread.start()
    try:
